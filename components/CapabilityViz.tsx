...
/* Build a paused timeline (scoped to `root`) once, then play/reset it as `active`
   toggles. `build` receives the root element so it can measure path lengths. */
function useViz(active: boolean, build: (root: HTMLDivElement) => gsap.core.Timeline | undefined) {
  ...
  useEffect(() => {
    ...
    // build is recreated per render but only its first run matters (deps intentionally empty).
...
function BrandViz({ color, active }: { color: string; active: boolean }) {
  const ref = useViz(active, (root) => {
    ...
    tl.fromTo(".bv-star", { strokeDashoffset: len, strokeDasharray: len, opacity: 0.25 },
      { strokeDashoffset: 0, opacity: 1, duration: 1.2, ease: "power2.inOut" }, 0);
    tl.fromTo(".bv-dot", { scale: 0, transformOrigin: "center" },
      { scale: 1, duration: 0.5, stagger: 0.16, ease: "back.out(2.6)" }, 0.35);
    tl.fromTo(".bv-ring", { scale: 0.7, opacity: 0, transformOrigin: "110px 90px" },
      { scale: 1, opacity: 0.5, duration: 1, ease: "power2.out" }, 0.1);
    return tl;
  });
  return (
    <div ref={ref} className={wrap}>
      <svg {...svgProps}>
        <g className="bv-spin">
          <circle className="bv-ring" cx="110" cy="90" r="60" fill="none" stroke={color} strokeWidth="1" opacity="0.5" />
          <circle className="bv-dot" cx="110" cy="28" r="4.5" fill={color} />
          <circle className="bv-dot" cx="162" cy="120" r="4.5" fill={color} />
          <circle className="bv-dot" cx="58" cy="120" r="4.5" fill={color} />
        </g>
        <g transform="translate(63,43) scale(0.94)">
          <polygon className="bv-star" points={STAR_POINTS} fill="none" stroke={INK} strokeWidth="2.2" strokeLinejoin="round" />
        </g>
      </svg>
    </div>
  );
...
function EditorialViz({ color, active }: { color: string; active: boolean }) {
  const ref = useViz(active, () => {
    ...
    tl.fromTo(".ev-cap", { scale: 0, rotation: -30, transformOrigin: "center" },
      { scale: 1, rotation: 0, duration: 0.5, ease: "back.out(2.2)" });
    tl.fromTo(".ev-line", { scaleX: 0, transformOrigin: "left center" },
      { scaleX: 1, duration: 0.5, stagger: 0.1, ease: "power3.out" }, "-=0.2");
    tl.to(".ev-line", { opacity: 0.22, duration: 0.35, stagger: 0.04 }, "+=0.8");
    tl.to(".ev-cap", { opacity: 0.22, duration: 0.35 }, "<");
    return tl;
  });
  const lines = [
...
function WebViz({ color, active }: { color: string; active: boolean }) {
  const ref = useViz(active, (root) => {
    ...
    tl.fromTo(".wv-win", { strokeDashoffset: len, strokeDasharray: len },
      { strokeDashoffset: 0, duration: 0.8, ease: "power2.inOut" });
    tl.fromTo(".wv-dot", { scale: 0, transformOrigin: "center" },
      { scale: 1, duration: 0.3, stagger: 0.07, ease: "back.out(2)" }, "-=0.35");
    tl.fromTo(".wv-block", { scaleY: 0, opacity: 0, transformOrigin: "top left" },
      { scaleY: 1, opacity: 1, duration: 0.4, stagger: 0.1, ease: "power2.out" }, "-=0.1");
    tl.fromTo(".wv-cursor", { x: 54, y: -16, opacity: 0 },
    ...
    tl.fromTo(".wv-ripple", { scale: 0, opacity: 0.6, transformOrigin: "center" },
  ...
  return (
    <div ref={ref} className={wrap}>
      <svg {...svgProps}>
        <rect className="wv-win" x="42" y="36" width="136" height="108" rx="9" fill="none" stroke={INK} strokeWidth="2" />
        ...
        <rect className="wv-block" x="54" y="64" width="112" height="30" rx="4" fill={color} opacity="0.85" />
        ...
        <path className="wv-cursor" d="M0 0 L0 16 L4.5 12 L7.5 18 L10 16.5 L7 11 L13 11 Z"
          transform="translate(72 122)" fill={PAPER} stroke={INK} strokeWidth="1.2" strokeLinejoin="round" />
      </svg>
    </div>
  );
}
...
function StudioViz({ color, active }: { color: string; active: boolean }) {
  const ref = useViz(active, () => {
    const tl = gsap.timeline();
    tl.fromTo([".av-c", ".av-s", ".av-t"], { scale: 0, transformOrigin: "center" },
      { scale: 1, duration: 0.6, stagger: 0.12, ease: "back.out(2)" }, 0);
    tl.fromTo(".av-sw", { scaleY: 0, transformOrigin: "bottom" },
      { scaleY: 1, duration: 0.5, stagger: 0.08, ease: "back.out(2.4)" }, 0.2);
  ...
  return (
    <div ref={ref} className={wrap}>
      <svg {...svgProps}>
        ...
        <rect className="av-s" x="120" y="44" width="42" height="42" rx="4" fill="none" stroke={INK} strokeWidth="2.4" />
...
function MotionViz({ color, active }: { color: string; active: boolean }) {
  const ref = useViz(active, () => {
    const tl = gsap.timeline();
    // two arcs sweeping at different speeds
    ...
    // bouncing ball with squash + stretch (classic 12 principles)
    ...
    s.to(".mv-shadow", { scaleX: 1.5, opacity: 0.5, transformOrigin: "center", duration: 0.5, ease: "power2.in" })
      .to(".mv-shadow", { scaleX: 0.8, opacity: 0.2, duration: 0.79, ease: "power2.out" });
    tl.add(b, 0).add(s, 0);
    return tl;
  });
  return (
    <div ref={ref} className={wrap}>
      <svg {...svgProps}>
        <circle className="mv-arc1" cx="110" cy="90" r="64" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeDasharray="70 230" opacity="0.7" />
        <circle className="mv-arc2" cx="110" cy="90" r="50" fill="none" stroke={INK} strokeWidth="2" strokeLinecap="round" strokeDasharray="40 274" opacity="0.5" />
        <ellipse className="mv-shadow" cx="110" cy="142" rx="22" ry="5" fill={INK} opacity="0.25" />
        <circle className="mv-ball" cx="110" cy="78" r="17" fill={color} />
      </svg>
    </div>
  );
}
...
function SocialViz({ color, active }: { color: string; active: boolean }) {
  const ref = useViz(active, () => {
    ...
    tl.fromTo(".sv-tile", { scale: 0, opacity: 0, transformOrigin: "center" },
      { scale: 1, opacity: 1, duration: 0.42, ease: "back.out(1.8)", stagger: { each: 0.05, grid: [3, 3], from: "center" } });
    tl.fromTo(".sv-heart", { scale: 0, transformOrigin: "center" },
      { scale: 1, duration: 0.45, ease: "back.out(3.5)" }, "+=0.15");
    tl.fromTo(".sv-spark", { scale: 0, opacity: 1, transformOrigin: "center" },
      { scale: 1, opacity: 0, duration: 0.55, ease: "power2.out", stagger: 0 }, "<0.05");
    tl.to(".sv-tile", { opacity: 0.28, duration: 0.35, stagger: { each: 0.03, grid: [3, 3], from: "edges" } }, "+=0.6");
    tl.to(".sv-heart", { opacity: 0.28, duration: 0.35 }, "<");
    return tl;
  });
  ...
  rows.forEach((y, r) => cols.forEach((x, c) => {
    const i = r * 3 + c;
    tiles.push({ x, y, fill: i % 3 === 0 ? color : i % 3 === 1 ? INK : color, o: i % 3 === 2 ? 0.4 : 1 });
  }));
