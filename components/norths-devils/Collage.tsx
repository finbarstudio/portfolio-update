...
import { collageMeta, type CollagePos } from "@/content/northsCollage";
...
// exact coordinates back to content/norths-collage.json (dev only).
...
export default function Collage({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      const inners = gsap.utils.toArray<HTMLElement>(section.querySelectorAll(".nd-para-inner"));
      const floats = gsap.utils.toArray<HTMLElement>(section.querySelectorAll(".nd-para-float"));
      const crest = section.querySelector<HTMLElement>(".nd-collage-crest");
      ...
      const scatters = gsap.utils.toArray<HTMLElement>(section.querySelectorAll(".nd-para-scatter"));
      scatters.forEach((el) => {
        const r = el.getBoundingClientRect();
  ...
  useEffect(() => {
    ...
    const imgs = Array.from(section.querySelectorAll<HTMLImageElement>("img.nd-para"));
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      sectionRef.current
        ?.querySelectorAll<HTMLElement>(".nd-para-inner, .nd-collage-crest")
        .forEach((el) => {
  ...
  const save = async () => {
    setSaved("saving");
    try {
      const res = await fetch("/api/norths-collage", {
        method: "POST",
  ...
  return (
    <section ref={sectionRef} id="one" className={`nd-collage${edit ? " nd-collage--edit" : ""}`}>
      {items.map((it) => {
        ...
        return (
          <div
            ...
            id={`nd-item-${it.key}`}
            data-key={it.key}
            className={`nd-para-wrap${isSel ? " is-sel" : ""}`}
            style={{
              left: `${it.x}%`,
              top: `${it.y}%`,
              width: `calc(${it.w}vw * var(--para-scale, 1))`,
              transform: it.rot ? `rotate(${it.rot}deg)` : undefined,
              // The hero image always rides on top of the pile.
              zIndex: isSel ? 20 : it.key === "hero" ? 10 : 1,
            }}
          ...
          >
            ...
            <div className="nd-para-inner" data-w={it.w}>
              <div className="nd-para-scatter">
                <div className="nd-para-float">
                  <img
                    src={meta.src}
                    alt={meta.alt}
                    className="nd-para"
            ...
            {isSel && (
              <>
                <span
                  className="nd-h nd-h-resize"
                  onPointerDown={(e) => startResize(e, it)}
                />
                <span
                  className="nd-h nd-h-rotate"
      ...
      <img
        src="/norths-devils/crest.png"
        alt="Norths Devils RLFC crest"
        className="nd-collage-crest"
        draggable={false}
      />

      {edit && collapsed && (
        <button className="nd-editor-chip" onClick={() => setCollapsed(false)}>
          ▸ Editor
        </button>
      )}

      {edit && !collapsed && (
        <div className="nd-editor" onPointerDown={(e) => e.stopPropagation()}>
          <div className="nd-editor-head">
            <span className="nd-editor-title">Collage editor</span>
            <button className="nd-editor-hide" onClick={() => setCollapsed(true)} title="Hide panel">
              Hide ▾
            </button>
          </div>
          {selItem ? (
            <>
              <div className="nd-editor-fields">
              ...
              <button className="nd-editor-del" onClick={() => remove(selItem.key)}>
                Delete “{selItem.key}” (or press ⌫)
              </button>
            </>
          ) : (
            <div className="nd-editor-hint">Click an item. Drag to move, corner = resize, top dot = rotate. Arrow keys nudge (Shift = bigger), ⌫ deletes.</div>
          )}
          <div className="nd-editor-row">
            <button onClick={save}>
