...
export default function HomeHeader() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.fromTo(
        logoRef.current,
        { opacity: 0, yPercent: -16, scale: 0.95 },
    ...
    const t = gsap.delayedCall(3.0, () => {
      window.dispatchEvent(new Event("mbc-prestige:intro-done"));
    });
  ...
  return (
    ...
    >
      <div ref={contentRef} className="flex flex-col items-center will-change-transform w-full">
        <div ref={logoRef} className="w-[clamp(140px,18vw,210px)]">
          <Image
            src="/mbc-prestige/logo.png"
            alt="MBC Prestige"
            width={200}
            height={200}
            priority
        ...
        <div className="mt-9 md:mt-12 flex flex-col items-center text-center">
          <div className="overflow-hidden">
            <p
              className="mbc-hero-line violet text-[var(--ink)]"
              style={{ fontSize: "clamp(1.1rem, 2.3vw, 1.9rem)", letterSpacing: "0.12em", fontWeight: 300 }}
            >
              MBC KNOW NOOSA BETTER THAN ANYONE
            </p>
          </div>
          <div className="overflow-hidden mt-5">
            <p
              className="mbc-hero-line text-[var(--ink-soft)]"
              style={{ fontSize: "clamp(0.6rem, 1.05vw, 0.78rem)", letterSpacing: "0.32em", textTransform: "uppercase" }}
            >
              Boutique Luxury Developments · Sunshine Coast
            </p>
          </div>
        </div>
      </div>
