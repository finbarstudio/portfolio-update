...
import ProjectLaurel from "@/components/mbc-prestige/ProjectLaurel";
...
const CATEGORIES = ["Apartments", "Land Releases", "Noosa"];

// Real MBC Prestige developments.
const PROJECTS: Item[] = [
  { slug: "kalani", title: "Kalani", location: "Noosaville", categories: ["Apartments", "Noosa"] },
  { slug: "sails", title: "Sails", location: "Sunshine Beach", categories: ["Apartments", "Noosa"] },
  { slug: "oceane", title: "Océane", location: "Coolum Beach", categories: ["Apartments"] },
  { slug: "botanica", title: "Botanica", location: "Valdora", categories: ["Land Releases"] },
  { slug: "habitat", title: "Habitat", location: "Palmwoods", categories: ["Land Releases"] },
  { slug: "tanglewood", title: "Tanglewood", location: "Old Tewantin", categories: ["Land Releases", "Noosa"] },
];
...
function GridTile({ p, eager }: { p: Item; eager: boolean }) {
  ...
  return (
    ...
    >
      ...
      />
      <Image
        src={`/mbc-prestige/projects/${p.slug}.webp`}
        alt={`${p.title}, MBC Prestige`}
        fill
        quality={82}
        priority={eager}
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/55 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
        <h3 className="violet text-white text-sm" style={{ letterSpacing: "0.12em" }}>
          {p.title.toUpperCase()}
        </h3>
