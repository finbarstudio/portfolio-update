...
export default function Nav({
  ...
  useEffect(() => {
    ...
    window.addEventListener("resolve-construction:intro-done", reveal);
    window.addEventListener("scroll", onScroll, { passive: true });
    const fallback = window.setTimeout(reveal, 5500);
    function cleanup() {
      window.removeEventListener("resolve-construction:intro-done", reveal);
    ...
    >
      <div className="flex md:grid md:grid-cols-[1fr_auto_1fr] items-center h-14 px-5 md:px-8">
        ...
        <div className="flex items-center">
          ...
          {showLogo && (
            <a
              href="/resolve-construction/site"
