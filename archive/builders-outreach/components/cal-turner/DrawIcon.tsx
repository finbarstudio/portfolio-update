...
export default function DrawIcon({ d, title, label, delay = 0 }: DrawIconProps) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.set(".ct-glyph-line", { yPercent: -120 });
  ...
  return (
    <div ref={rootRef} className="flex flex-col items-center gap-1.5 w-[86px] shrink-0">
      ...
      <div className="overflow-hidden">
        <span className="ct-glyph-line block violet text-[8px] tracking-[0.14em] uppercase leading-none text-center [text-indent:0.14em] text-[var(--black)]">
          {title}
        </span>
      </div>

      <div className="overflow-hidden">
        <span className="ct-glyph-line block violet text-[var(--ink)]/70 text-[7.5px] tracking-[0.12em] uppercase leading-tight text-center [text-indent:0.12em]">
          {label}
        </span>
      </div>
    </div>
